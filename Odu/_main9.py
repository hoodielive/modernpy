...
def start_quiz(statements):
    ...
    for statement in statements:
        ...
        # Replace the word with [Hidden]
        hidden_statement = statement_text.replace(statement_word, "[Hidden]", 1)

        user_answer = input(f"\nStatement:\n{number}. {hidden_statement}\nEnter the correct word (Odu): ")

        if user_answer.strip() == statement_word:
            print("Correct!")
            correct_answers += 1
        else:
            print(f"Wrong. The correct word (Odu) is: {statement_word}")
